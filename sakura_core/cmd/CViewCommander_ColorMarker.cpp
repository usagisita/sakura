...
static bool GetSCMFileName(CEditDoc* pcEditDoc, LPCWSTR pszFileName, bool bSave, std::wstring& file_str)
{
...
void CViewCommander::Command_COLORMARKER_LOAD(LPCWSTR pszFileName)
{
	...
	if( GetSCMFileName(GetDocument(), pszFileName, false, file_str) ){
		if( CColorMarkerVisitor().LoadColorMarker(&GetDocument()->m_cDocLineMgr, file_str.c_str()) ){
			GetDocument()->m_pcEditWnd->Views_Redraw();
		}else{
			ErrorMessage(m_pCommanderView->GetHwnd(), LS(STR_ERR_LOADCOLORMAKER), file_str.c_str());
		}
	}
}
...
{
	std::wstring file_str;
	if( GetSCMFileName(GetDocument(), pszFileName, true, file_str) ){
		if( CColorMarkerVisitor().SaveColorMarker(&GetDocument()->m_cDocLineMgr, file_str.c_str()) ){
		}else{
			ErrorMessage(m_pCommanderView->GetHwnd(), LS(STR_ERR_SAVECOLORMAKER), file_str.c_str());
		}
	}
}
