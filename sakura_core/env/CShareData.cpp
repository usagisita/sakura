...
//	To Here Sept. 19,2000
		...
		// [ウィンドウ]タブ
		{
			...
			wcscpy_fix( sWindow.m_szWindowCaptionActive, 
				L"${w?$h$:アウトプット$:${I?$f$n$:$N$n$}$}${U?(更新)$} -"
				L" $A $V ${R?(ビューモード)$:(上書き禁止)$}${M?  【キーマクロの記録中】$} $<profile>" );
			wcscpy_fix( sWindow.m_szWindowCaptionInactive, 
		...
		// [書式]タブ
		{
			...
			/* 見出し記号 */
			wcscpy_fix( sFormat.m_szMidashiKigou, L"１２３４５６７８９０（(［[「『【■□▲△▼▽◆◇○◎●§・※☆★第①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩ一二三四五六七八九十壱弐参伍" );
			/* 引用符 */
			wcscpy_fix( sFormat.m_szInyouKigou, L"> " );		/* 引用符 */
			...
			*/
			...
			wcscpy_fix( sFormat.m_szDateFormat, L"yyyy\'年\'M\'月\'d\'日(\'dddd\')\'" );	//日付書式
			sFormat.m_nTimeFormatType = 0;	//時刻書式のタイプ
			wcscpy_fix( sFormat.m_szTimeFormat, L"tthh\'時\'mm\'分\'ss\'秒\'"  );			//時刻書式
		}
		...
		// [ファイル名表示]タブ
		{
			...
			wcscpy_fix( sFileName.m_szTransformFileNameFrom[0], L"%DeskTop%\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameTo[0],   L"デスクトップ\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameFrom[1], L"%Personal%\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameTo[1],   L"マイドキュメント\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameFrom[2], L"%Cache%\\Content.IE5\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameTo[2],   L"IEキャッシュ\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameFrom[3], L"%TEMP%\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameTo[3],   L"TEMP\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameFrom[4], L"%Common DeskTop%\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameTo[4],   L"共有デスクトップ\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameFrom[5], L"%Common Documents%\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameTo[5],   L"共有ドキュメント\\" );
			wcscpy_fix( sFileName.m_szTransformFileNameFrom[6], L"%AppData%\\" );	// 2007.05.19 ryoji 追加
			wcscpy_fix( sFileName.m_szTransformFileNameTo[6],   L"アプリデータ\\" );	// 2007.05.19 ryoji 追加
