...
void CShareData_IO::ShareData_IO_Common( CDataProfile& cProfile )
{
	...
	struct ShareData_IO_ColorMarker{
		bool IO_ColorMarker(CDataProfile& cProfile, LPCWSTR pszSecName, LPCWSTR pszKey, CMarkerItem& item){
			...
			if( cProfile.IsReadingMode() ){
				...
				buf[0] = L'\0';
				ret = cProfile.IOProfileData(pszSecName, pszKey, StringBufferW(buf));
				if( ret ){
	...
	for( int i = 0; i < _countof(sMarker.m_ColorItems); i++ ){
		...
		cProfile.IOProfileData(pszSecName, szKey, StringBufferW(sMarker.m_szSetNames[i]));
	}
	for( int i = 0; i < _countof(sMarker.m_dwChooseColor); i++ ){
		...
		if( cProfile.IsReadingMode() ){
			wchar_t buf[100];
			buf[0]=L'\0';
			ret = cProfile.IOProfileData(pszSecName, szKey, StringBufferW(buf));
			if( ret ){
...
void CShareData_IO::ShareData_IO_MainMenu( CDataProfile& cProfile )
{
	...
	if( cProfile.IsReadingMode() && nVersion < nCurrentVer ){
		...
		SMainMenuAddItemInfo addInfos[] = {
			...
			{1, F_FUNCLIST_NEXT, F_JUMPHIST_SET, L'\0', true, false, false, 1, 0}, 	// 次の関数リストマーク(セパレータ追加)
			{1, F_FUNCLIST_PREV, F_FUNCLIST_NEXT, L'\0', false, false, false, 1, 0}, 	// 前の関数リストマーク
			{1, F_MODIFYLINE_NEXT, F_FUNCLIST_PREV, L'\0', false, false, false, 1, 0}, 	// 次の変更行へ
			{1, F_MODIFYLINE_PREV, F_MODIFYLINE_NEXT, L'\0', false, false, false, 1, 0}, 	// 前の変更行へ
			{1, F_MODIFYLINE_NEXT_SEL, F_GOFILEEND_SEL, L'\0', true, false, false, 1, 0}, 	// (選択)次の変更行へ
			{1, F_MODIFYLINE_PREV_SEL, F_MODIFYLINE_NEXT_SEL, L'\0', false, false, false, 1, 0}, 	// (選択)前の変更行へ
