...
void CDocOutline::MakeFuncList_ColorMarker( CFuncInfoArr* pcFuncInfoArr )
{
	...
	// マーカー全部に"テキスト"と表示されるうっとうしいのでやめる
	// pcFuncInfoArr->SetAppendText( CM_INFO_TAG_STRING,   LS(STR_MARKER_TAG_STRING),   false );

	pcFuncInfoArr->SetAppendText( CM_INFO_TAG_GYOU,     LS(STR_MARKER_TAG_GYOU),     false );
	pcFuncInfoArr->SetAppendText( CM_INFO_TAG_LINE,     LS(STR_MARKER_TAG_LINE),     false );
	pcFuncInfoArr->SetAppendText( CM_INFO_TAG_GYOULINE, LS(STR_MARKER_TAG_GYOULINE), false );
