...
void CShareData::InitTypeConfigs(DLLSHAREDATA* pShareData, std::vector<STypeConfig*>& types )
{
	...
	for(int i = 0; i < _countof(table) && i < MAX_TYPES; i++){
		...
		wcscpy_fix(pShareData->m_TypeMini[i].m_szTypeExts, type->m_szTypeExts);
		wcscpy_fix(pShareData->m_TypeMini[i].m_szTypeName, type->m_szTypeName);
...
/************************/
	...
	wcscpy_fix( pType->m_szTabViewString, _EDITL("^       ") );	/* TAB表示文字列 */
	...
	wcscpy_fix( pType->m_szKinsokuKuto, L"、。，．､｡,." );	// 句読点ぶら下げ文字	// 2009.08.07 ryoji
